...
$src = Join-Path $repoRoot 'adapters\RE-Engine'
$build = Join-Path $src 'build'
if ($Clean) { if (Test-Path $build) { Remove-Item -Recurse -Force $build } }

$gen = Get-Generator
$ts = New-BuildTimestamp
$logDir = Join-Path $repoRoot "logs\build\RE-Engine\$ts"
...
# Locate artifact
$dll = Get-ChildItem -LiteralPath $build -Recurse -Filter 'RE_Engine_Adapter.dll' -ErrorAction SilentlyContinue | Select-Object -First 1 -Expand FullName
$artifacts = @()
if ($dll) { $artifacts += $dll }

# Deploy to profile
$profileDir = Join-Path $repoRoot 'profiles\re7'
...
Write-BuildSummary -SummaryPath $summary -ProjectName 'RE-Engine Adapter' -Engine 'RE Engine (RE7)' -Config $Config -Generator $genLabel -BuildDir $build -ConfigureLog $configureLog -BuildLog $buildLog -WarnCount $warns -ErrCount $errs -Artifacts $artifacts
