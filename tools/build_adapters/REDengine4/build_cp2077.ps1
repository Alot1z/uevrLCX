...
$src = Join-Path $repoRoot 'adapters\REDengine4'
$build = Join-Path $src 'build'
if ($Clean) { if (Test-Path $build) { Remove-Item -Recurse -Force $build } }

$gen = Get-Generator
$ts = New-BuildTimestamp
$logDir = Join-Path $repoRoot "logs\build\REDengine4\$ts"
...
# Locate artifact (multi-config layout)
$dll = Get-ChildItem -LiteralPath $build -Recurse -Filter 'redengine4_adapter.dll' -ErrorAction SilentlyContinue | Select-Object -First 1 -Expand FullName
$artifacts = @()
if ($dll) { $artifacts += $dll }

# Deploy to profile
$profileDir = Join-Path $repoRoot 'profiles\Cyberpunk2077'
if ($dll -and (Test-Path $profileDir)) {
  Copy-Item -Force -LiteralPath $dll -Destination $profileDir
  $artifacts += (Join-Path $profileDir (Split-Path $dll -Leaf))
}

$genLabel = if ($gen) { $gen } else { 'Default' }
Write-BuildSummary -SummaryPath $summary -ProjectName 'REDengine4 Adapter' -Engine 'REDengine 4 (Cyberpunk 2077)' -Config $Config -Generator $genLabel -BuildDir $build -ConfigureLog $configureLog -BuildLog $buildLog -WarnCount $warns -ErrCount $errs -Artifacts $artifacts
