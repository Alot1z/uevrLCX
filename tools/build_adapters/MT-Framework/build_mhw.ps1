...
$src = Join-Path $repoRoot 'adapters\MT-Framework'
$build = Join-Path $src 'build'
if ($Clean) { if (Test-Path $build) { Remove-Item -Recurse -Force $build } }

$gen = Get-Generator
$ts = New-BuildTimestamp
$logDir = Join-Path $repoRoot "logs\build\MT-Framework\$ts"
...
# Locate artifact
$dll = Get-ChildItem -LiteralPath $build -Recurse -Filter 'mt_framework_cross_engine_adapter.dll' -ErrorAction SilentlyContinue | Select-Object -First 1 -Expand FullName
$artifacts = @()
if ($dll) { $artifacts += $dll }

# Deploy to profile
$profileDir = Join-Path $repoRoot 'profiles\MonsterHunterWorld'
...
Write-BuildSummary -SummaryPath $summary -ProjectName 'MT-Framework Adapter' -Engine 'MT Framework (Monster Hunter: World)' -Config $Config -Generator $genLabel -BuildDir $build -ConfigureLog $configureLog -BuildLog $buildLog -WarnCount $warns -ErrCount $errs -Artifacts $artifacts
